...
import argparse, os
from modules import attmil,clam,rps,transmil,mean_max
...
def main(args):
    ...
    if not hasattr(args, 'save_patch_scores'):
        args.save_patch_scores = True
    if not hasattr(args, 'patch_scores_dir'):
        args.patch_scores_dir = './patch_index/'

    parser.add_argument('--save_patch_scores', action='store_true', 
                        help='patch_scores')
    parser.add_argument('--patch_scores_dir', type=str, default='./patch_index/', 
                        help='patch_scores_dir')

    parser.add_argument('--k_top_patches', default=0.3, type=float, 
                        help='k_top_patches')
    parser.add_argument('--top_k', default=3, type=int, 
                        help='top_k')
    parser.add_argument('--save_top_patches', action='store_true', 
                        help='save top k patches for each class')
    ...
    for k in range(args.fold_start, args.cv_fold):
        ...
        if not args.no_log:
            print(f'Fold {k} best results - Accuracy: {acs[-1]:.3f}, AUC: {auc[-1]:.3f}, Precision: {pre[-1]:.3f}, Recall: {rec[-1]:.3f}, F-score: {fs[-1]:.3f}')
...
def one_fold(args,k,ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    ...
    if args.model == 'rps':
        ...
        if not hasattr(args, 'k_top_patches'):
            args.k_top_patches = 0.3
        ...
        model_params = {
            ...
            'temp_t': args.temp_t if hasattr(args, 'temp_t') else 0.1,
            'temp_s': args.temp_s if hasattr(args, 'temp_s') else 1.0,
            'act': args.act,
            'head': args.n_heads,
            'da_act': args.da_act,
            ...
            'top_k': args.top_k,
            'select_mask': True,  
            'use_text_guidance': getattr(args, 'use_text_guidance', True),
            'text_features_path': getattr(args, 'text_features_path', './text_features.pt'),
        }
        
        model = rps.RPS(**model_params).to(device)
            
    elif args.model == 'pure':
        model = rps.RPS(select_mask=False,n_classes=args.n_classes,act=args.act,head=args.n_heads,da_act=args.da_act,baseline=args.baseline).to(device)
    ...
    for epoch in range(epoch_start, args.num_epoch):
        ...
        if model_tea is not None:
            _,accuracy_tea, auc_value_tea, precision_tea, recall_tea, fscore_tea, test_loss_tea, threshold_optimal_tea = val_loop(args,model_tea,val_loader,device,criterion,None,epoch,model_tea)
            if args.wandb:
                rowd = OrderedDict([
...
def train_loop(args,model,model_tea,loader,optimizer,device,amp_autocast,criterion,loss_scaler,scheduler,k,mm_sche,epoch):
    ...
    save_patch_scores = getattr(args, 'save_patch_scores', True)
    patch_scores_dir = getattr(args, 'patch_scores_dir', './patch_index/')

    if args.save_patch_scores:
        os.makedirs(args.patch_scores_dir, exist_ok=True)

    for i, data in enumerate(loader):
        optimizer.zero_grad()

        if len(data) == 3:  
            features_data, label, slide_names = data
        else:  
            features_data, label = data
            slide_names = None

        if isinstance(features_data,(list,tuple)):
            for j in range(len(features_data)):
                features_data[j] = features_data[j].to(device)
            bag = features_data
            batch_size = features_data[0].size(0)
        else:
            bag = features_data.to(device)  # b*n*1024
        ...
        if args.model == 'rps':
            if model_tea is not None:
                teacher_outputs = model_tea.forward_test(bag)
                

                if isinstance(teacher_outputs, tuple):
                    if len(teacher_outputs) >= 4:  
                        score_matrix = teacher_outputs[3]  
                    else:
                        score_matrix = None
                elif isinstance(teacher_outputs, dict):
                    score_matrix = teacher_outputs.get('score_matrix', None)
                else:
                    score_matrix = None
                
                if score_matrix is None:
                    print("Warning: No score_matrix obtained from teacher model")

                model_outputs = model(bag, score_matrix=score_matrix, save_top_patches=True)

                if isinstance(model_outputs, tuple) and len(model_outputs) >= 5:
                    logits, cls_loss, patch_num, keep_num, top_patches_dict = model_outputs
                elif isinstance(model_outputs, tuple) and len(model_outputs) == 4:
                    logits, cls_loss, patch_num, keep_num = model_outputs
                    top_patches_dict = None
                ...
                if save_patch_scores and score_matrix is not None:
                    try:
                        current_slide_names = slide_names if slide_names is not None else [f"sample_{i}"]
                        
                        os.makedirs(patch_scores_dir, exist_ok=True)
                        
                        if args.datasets.lower() == 'camelyon16' and args.n_classes == 2:
                            class_names = ['Normal', 'Tumor']
                        else:
                            class_names = [f'Class_{i}' for i in range(args.n_classes)]
                        
                        patch_data = {
                            'patch_num': float(patch_num) if hasattr(patch_num, 'item') else patch_num,
                            'keep_num': float(keep_num) if hasattr(keep_num, 'item') else keep_num,
                            'class_names': class_names,
                            'top_patches_dict': top_patches_dict if top_patches_dict is not None else {}
                        }
                        
                        for idx, slide_name in enumerate(current_slide_names):
                            clean_slide_name = str(slide_name).replace('/', '_').replace('\\', '_')
                            save_path = os.path.join(patch_scores_dir, f'epoch_{epoch}_batch_{i}_slide_{clean_slide_name}_patch_scores.pt')
                            
                            slide_patch_data = patch_data.copy()
                            slide_patch_data['slide_name'] = slide_name
                            slide_patch_data['batch_sample_idx'] = idx
                            
                            torch.save(slide_patch_data, save_path)
                    
                    except Exception as e:
                        print(f"Error saving patch scores: {e}")
                        import traceback
                        traceback.print_exc()
                else:
                    print(f"Debug: Skipping save - save_patch_scores={save_patch_scores}, score_matrix={'not None' if score_matrix is not None else 'None'}")
                
                cls_tea = None if args.cl_alpha == 0. else cls_tea
            
            else:
                model_outputs = model(bag, save_top_patches=True)
        ...
        if args.loss == 'ce':
            ...
            if isinstance(logits, tuple):
                logit_loss = criterion(logits[0].view(batch_size, -1), one_hot(label.view(batch_size, -1).float(), num_classes=2))
            else:
...
def val_loop(args,model,loader,device,criterion,early_stopping,epoch,model_tea=None):
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            if args.model in ('rps','pure'):
                test_logits = model.forward_test(bag)
            else:
                test_logits = model(bag)

            if args.loss == 'ce':
                if (args.model == 'rps' and isinstance(test_logits,(list,tuple))):
                    test_loss = criterion(test_logits[0].view(batch_size,-1),label)
                    softmax_output = torch.softmax(test_logits[0],dim=-1)
                    if softmax_output.shape[-1] == 2:  # 二分类
                        bag_logit.append(softmax_output[:,1].cpu().squeeze().numpy())
                    else:  #
                        bag_logit.append(softmax_output.cpu().squeeze().numpy())
                else:
                    logits_tensor = test_logits['bag_logits'] if isinstance(test_logits, dict) else test_logits
                    test_loss = criterion(logits_tensor.view(batch_size,-1),label)
                    if batch_size > 1:
                        bag_logit.extend(torch.softmax(logits_tensor,dim=-1).cpu().squeeze().numpy())
                    else:
                        bag_logit.append(torch.softmax(logits_tensor,dim=-1).cpu().squeeze().numpy())
            elif args.loss == 'bce':
                logits_tensor = test_logits['bag_logits'] if isinstance(test_logits, dict) else test_logits
                test_loss = criterion(logits_tensor.view(batch_size,-1),label.view(1,-1).float())
                bag_logit.append(torch.sigmoid(logits_tensor).cpu().squeeze().numpy())

            loss_cls_meter.update(test_loss,1)
        
        if isinstance(bag_logit[0], np.ndarray) and len(bag_logit[0].shape) > 0:
            bag_logit = np.vstack(bag_logit)
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                ...
                if args.loss == 'ce':
                    if (args.model == 'rps' and isinstance(test_logits,(list,tuple))):
                        test_loss = criterion(test_logits[0].view(batch_size,-1),label)
                        bag_logit.append(torch.softmax(test_logits[0],dim=-1)[:,1].cpu().squeeze().numpy())
                    else:
                        logits_tensor = test_logits['bag_logits'] if isinstance(test_logits, dict) else test_logits
                        test_loss = criterion(logits_tensor.view(batch_size,-1),label)
                        if batch_size > 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epoch', default=130, type=int, help='Number of total training epochs [200]')
    ...
    parser.add_argument('--model', default='rps', type=str, help='Model name')
    ...
    parser.add_argument('--use_text_guidance', action='store_true', 
                        help='use_text_guidance')
    parser.add_argument('--text_features_path', type=str, 
                        default='./text_features.pt',
                        help='text_feature_path')
    # Our
