...
class C16Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Args
        :param idx: the index of item
        :return: features, label, and slide_name
        """
        if self.persistence:
            ...
            try:
                features = torch.load(file_path, encoding='utf-8-sig')
            except FileNotFoundError:
                alt_path = os.path.normpath(file_path)
                features = torch.load(alt_path, encoding='utf-8-sig')

        label = int(self.slide_label[idx])
        slide_name = self.file_name[idx]  
